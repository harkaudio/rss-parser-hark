...
class Parser {
  ...
  parseURL(feedUrl, callback, redirectCount=0) {
    ...
    */
    ...
    let prom = new Promise((resolve, reject) => {
      let req = axios.get(urlParts, {
        // headers:headers,
        // auth:urlParts.auth,
        responseType: 'stream'
      })
      .then((res) => {
        console.log('axios parsing feedurl', feedUrl);
        if (this.options.maxRedirects && res.statusCode >= 300 && res.statusCode < 400 && res.headers['location']) {
          if (redirectCount === this.options.maxRedirects) {
            return reject(new Error("Too many redirects"));
          } else {
  ...
  buildRSS(channel, items) {
    console.log('buildRSS', Object.keys(channel))
    items = items || [];
    let feed = {items: []};
    let feedFields = fields.feed.concat(this.options.customFields.feed);
    let itemFields = fields.item.concat(this.options.customFields.item);
    if(channel['atom:link']){ 
      console.log('ALL href array present111', channel['atom:link'])
      for( let item of channel['atom:link'] ){
        if(item.$.rel == "self" ){
          feed.feedUrl = item.$.href;
          console.log('new self href', feed.feedUrl)
          break;
        }
      }
      
    }else{
      console.log('All href array not present111')
    }
  ...
  decorateItunes(feed, channel) {
    let items = channel.item || [],
        entry = {};
    feed.itunes = {}
    console.log('decorateItunes', Object.keys(channel))
